...
function Footer() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/content/payment-icons`)
      .then(r => r.json())
      .then(d => setPaymentIcons(d.icons || []))
      .catch(() => {});
  }, []);
  ...
  return (
    <footer className="login-footer">
      <div className="login-footer-inner">
        ...
        <div>
          <p className="login-footer-col-title">Legal Info</p>
          <div className="login-footer-links">
      ...
      <div className="login-footer-bottom">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          <span style={{ fontSize: '12px', color: '#9ca3af' }}>Secure Payments:</span>
            ...
            {paymentIcons && paymentIcons.length > 0 ? (
              paymentIcons.map((icon, i) => (
                <img 
                  ...
                  src={icon.url || icon} 
                  alt={`pay-${i}`} 
                  style={{ height: '24px', width: 'auto', objectFit: 'contain', background: '#fff', borderRadius: '4px', padding: '2px' }} 
                />
              ))
            ) : (
              <>
